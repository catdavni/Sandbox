...
internal sealed class Plane : IDrawable
{
    ...
    public Plane(Device device, IResourceFactory resourceFactory)
    {
        ...
        var compiledVertexShader = resourceFactory.EnsureVertexShader(device, "plane.hlsl", "VShader");
        ...
        _pixelShader = resourceFactory.EnsurePixelShader(device, "plane.hlsl", "PShader");
    ...
    public DrawPipelineMetadata Draw(DrawPipelineMetadata previous, Device device)
    {
        ...
        currentMetadata = currentMetadata.EnsureVertexBufferBinding<PlaneVertices>(device, _vertexBuffer);
...
file static class AuxiliaryData
{
    ...
    public static readonly (string Key, PlaneVertices[] Data) Vertices =
        ($"{nameof(Plane)}_Vertices",
            new[]
            {
                new PlaneVertices(new RawVector3(-1, -1, -1), new RawVector2(0, 1)),
                new PlaneVertices(new RawVector3(-1, 1, -1), new RawVector2(0, 0)),
                new PlaneVertices(new RawVector3(1, 1, -1), new RawVector2(1, 0)),
                new PlaneVertices(new RawVector3(1, -1, -1), new RawVector2(1, 1))
            });
...
file readonly record struct PlaneVertices(RawVector3 Vertex, RawVector2 TexCoord);